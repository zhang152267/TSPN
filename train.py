...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    epoch_loss = []
    luck = 0
    if (epoch+1) <= 100 or ((epoch+1)%10==9):
        luck = 0
        print("************1**************")
        model.pre.requires_grad = True
        model.dab.requires_grad = True
    else:
        luck = 1
        model.pre.requires_grad = False
        model.dab.requires_grad = True
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        start_time = time.time()
        images, images1, labels, _, _ = batch
        images = Variable(images).cuda()
        images1 = Variable(images1).cuda()
        labels = Variable(labels.long()).cuda()
        output, pre_output = model(images)
        loss_1 = torch.mean((images1 - pre_output) ** 2)/200
        if luck == 0:
            loss_2 = 0
        else:
            loss_2 = criterion(output, labels)
        loss = loss_2 + loss_1
        scheduler.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="TSPN", help="model name: TSPN")
    parser.add_argument('--dataset', default="camvid", help="dataset: camvid")
