...
class CamVidDataSet(data.Dataset):
    ...
    def __init__(self, root='', list_path='', max_iters=None, crop_size=(360, 360),
        ...
        for name in self.img_ids:
            ...
            img1_file = img_file[:22] + "1" + img_file[22:]
            if 'val' in img_file:
                img1_file = img_file[:20] + "1" + img_file[20:]
            #print(img1_file)
    ...
    def __getitem__(self, index):
        ...
        image1 = cv2.imread(datafiles["img1"], cv2.IMREAD_COLOR)
        ...
        if self.scale:
            ...
            image1 = cv2.resize(image1, None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_LINEAR)
        ...
        if pad_h > 0 or pad_w > 0:
            img_pad = cv2.copyMakeBorder(image, 0, pad_h, 0,
                                         ...
                                         value=(0.0, 0.0, 0.0))
            img1_pad = cv2.copyMakeBorder(image1, 0, pad_h, 0,
                                         pad_w, cv2.BORDER_CONSTANT,
                                         value=(0.0, 0.0, 0.0))
            ...
            img_pad, img1_pad, label_pad = image, image1, label
        ...
        image1 = np.asarray(img1_pad[h_off: h_off + self.crop_h, w_off: w_off + self.crop_w], np.float32)
        ...
        return image.copy(), image1.copy(), label.copy(), np.array(size), name
